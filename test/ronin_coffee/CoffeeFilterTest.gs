...
class CoffeeFilterTest extends TestCase {

  function testBasicCompilation() {
    var filter = new CoffeeFilter()
    var out = new MockResponse()
    filter.doFilter(new MockRequest() {:MockPathInfo = "/example.coffee", :MockSource = "1"}, out, null)
  ...
  function testBasicCompilationWithCaching() {
    var filter = new CoffeeFilter() { :Cache = true }
    var out = new MockResponse()
    filter.doFilter(new MockRequest() {:MockPathInfo = "/example.coffee", :MockSource = "1"}, out, null)
    assertEquals("(function() {\n  1;\n}).call(this);\n", out.Output)
    assertEquals(1, filter.Compilations.intValue())
